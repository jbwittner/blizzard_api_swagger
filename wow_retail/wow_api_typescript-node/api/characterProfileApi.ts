...
export enum CharacterProfileApiApiKeys {
}

export class CharacterProfileApi {
    ...
    public setApiKey(key: CharacterProfileApiApiKeys, value: string) {
        (this.authentications as any)[CharacterProfileApiApiKeys[key]].apiKey = value;
    }
     ...
     */
    public async getCharacter (namespace: 'profile-eu' | 'profile-us' | 'profile-kr' | 'profile-tw' | 'profile-cn', region: 'eu' | 'us' | 'kr' | 'tw' | 'cn', realmSlug: string, characterName: string, locale?: 'en_US' | 'es_MX' | 'pt_BR' | 'de_DE' | 'en_GB' | 'es_ES' | 'fr_FR' | 'it_IT' | 'ru_RU' | 'ko_KR' | 'zh_TW' | 'zh_CN', options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CharacterData;  }> {
        const localVarPath = this.basePath + '/profile/wow/character/{realmSlug}/{characterName}'
        ...
        if (namespace === null || namespace === undefined) {
            throw new Error('Required parameter namespace was null or undefined when calling getCharacter.');
        }

        // verify required parameter 'region' is not null or undefined
        if (region === null || region === undefined) {
            throw new Error('Required parameter region was null or undefined when calling getCharacter.');
        }

        // verify required parameter 'realmSlug' is not null or undefined
        if (realmSlug === null || realmSlug === undefined) {
            throw new Error('Required parameter realmSlug was null or undefined when calling getCharacter.');
        }

        // verify required parameter 'characterName' is not null or undefined
        if (characterName === null || characterName === undefined) {
            throw new Error('Required parameter characterName was null or undefined when calling getCharacter.');
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: CharacterData;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "CharacterData");
